 ...
 *          2001/09/13   DA  Use specified experimental area for header
 *          2001/11/14   DA  Make output files default to current working directory. Add mud2col: multi-column mud2txt.
 *          2002/07/17   DA  Accept output file path, using input name
 *          2003/11/25   DA  Add -h option for selecting histograms
 *          2007/02/03   DA  Incorporate ral2mud and psi2mud for (old) RAL and PSI data files
 *          2010/08/26   DA  Release under GNU LGPL (mostly)
 *                           Changes for 64-bit Linux, especially for larger time_t (change time handling)
 *                           Always use private version of strndup
 *          2015/03/18   TW  Replace register with const
 *          2017/06/27   DA  Eliminate compile warning
...
#else
          printf( "MUDutil> " );
          if ( fgets(line,200,stdin) == NULL ) return( 0 );
#endif
          p = line;
          aargc = 1;
          while( 1 )
 ...
 *	None.
 *
 * NOT USED NOW! MAYBE WE WANT TO ALLOW WILDCARDS ON FILE NAMES!
 *
 *----------------------------------------------------------------------
 */
