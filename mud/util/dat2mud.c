...
/*
 * Read old LCR dat header. Returns the number of header blocks read (0 on failure)
 */
static int
    ...
    MUD_SEC_GRP* pMUD_indVarGrp;
{
    ...
    if( nBlocks <= 0 ) return ( FAILURE );
    pImusr_blk = (IMUSR_BLK*)zalloc( nBlocks*512 );
    if( (fread( pImusr_blk, 512, nBlocks, fin ) < nBlocks) ) return( FAILURE );
    if( pImusr_hdr->ncamp > 0 )
    {
      /*
