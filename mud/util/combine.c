 ...
 *   2001/07/04   DA  Add combine command to add/subtract/concatenate runs
 *   2009/08/19   DA  Improve handling of mismatched logged vars, avoiding a possible memory access error.
 *   2009/08/23   DA  Fix wrong bytes-per-bin for time-integral.
 *   2010/08/26   DA  Release under the GNU LGPL (Yes, LGPL)
 *   06-May-2021  DA  Silence some compile warnings
 */
...
int combine_mud (char* inFile, char* addFile, char* outFile, int iAddSub )
{
    int status = SUCCESS;
    UINT32 run_fmt_ID;
    FILE* fin  = NULL;
 ...
 *     Mainly take the information from the first run, ignoring the
 *     second, but do combine the timeBegin, timeEnd, elapsedSec.
 */

int combine_GEN_RUN_DESC( 
    ...
    int   iAddSub )
{
    ...
    BOOL haveBoth;
    BOOL didWarn = FALSE;
    MUD_SEC_GEN_SCALER* pMUD_inscal = NULL;
    MUD_SEC_GEN_SCALER* pMUD_addscal = NULL;
    MUD_SEC_GEN_SCALER* pMUD_outscal = NULL;
    ...
    int   iAddSub )
{
    ...
    int nh ;
    int nb = 0;
    MUD_SEC_GEN_HIST_HDR* pMUD_inhistHdr = NULL;
    MUD_SEC_GEN_HIST_HDR* pMUD_addhistHdr = NULL;
    MUD_SEC_GEN_HIST_HDR* pMUD_outhistHdr = NULL;
    MUD_SEC_GEN_HIST_DAT* pMUD_inhistDat = NULL;
    MUD_SEC_GEN_HIST_DAT* pMUD_addhistDat = NULL;
    MUD_SEC_GEN_HIST_DAT* pMUD_outhistDat = NULL;
	...
	MUD_SEC_GEN_HIST_unpack( nb, pMUD_addhistHdr->bytesPerBin,
        ...
        for( j = 0; j < nb; j++ )
        {   /* separate for dumb compiler warning */
    ...
    int iAddSub )
{
     ...
     */
    for( j=0; j<2; j++) // foreach pMUD_cmtGrp in {pMUD_incmtGrp, pMUD_addcmtGrp}
    {
...
int combine_IND_VAR(
    ...
    int iAddSub )
{
    MUD_SEC_GEN_IND_VAR* pMUD_inindVarHdr = NULL;
    MUD_SEC_GEN_IND_VAR* pMUD_addindVarHdr = NULL;
    MUD_SEC_GEN_IND_VAR* pMUD_outindVarHdr = NULL;
    int nvout = 0;
    double d, s;
    int i;
    ...
    int iAddSub )
{
    ...
    MUD_SEC_GEN_IND_VAR* pMUD_addindVarHdr = NULL;
    MUD_SEC_GEN_IND_VAR* pMUD_outindVarHdr = NULL;
    MUD_SEC_GEN_ARRAY* pMUD_inindVarDat = NULL;
    MUD_SEC_GEN_ARRAY* pMUD_addindVarDat = NULL;
    MUD_SEC_GEN_ARRAY* pMUD_outindVarDat = NULL;
    float* poutindVarData = NULL;
    float* poutj = NULL;
    int num, nvar;
    int nvout = 0;
    double sum, sum2, sum3, val, dn, mean, stddev;
    int i, j;
