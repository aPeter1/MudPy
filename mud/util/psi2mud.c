...
int PSI_readHdr( FILE* fin, PSI_INFO_HDR* pPSI_info_hdr );
...
int PSI_readHdr( FILE* fin, PSI_INFO_HDR* pPSI_info_hdr )
{
  ...
  char tempString[256];
  char *p;
  int i, j, count, tbin0,tbin1,tbin2, tbintemp, modbins;
  int runnumber;
...
//  p = buf+88;
  ...
  if(verbosemode == 1) for(i=0; i<=15; i++) printf("%8li ", (long int)pPSI_info_hdr->CNTOLD[i]);
  if(verbosemode == 1) printf("\n");

  p = buf+360;
  bdecode_obj( p, pPSI_info_hdr->I4SCAL_B, 12*4 );
  if(verbosemode == 1) printf("I4SCAL_B =");
  if(verbosemode == 1) for(i=0; i<=11; i++) printf("%10li ", (long int)pPSI_info_hdr->I4SCAL_B[i]);
  if(verbosemode == 1) printf("\n");
 
  p = buf+424;
  bdecode_4( p, &pPSI_info_hdr->TOTOLD );
  if(verbosemode == 1) printf("TOTOLD = %li \n", (long int)pPSI_info_hdr->TOTOLD);  
...
*/
  ...
  if(verbosemode == 1) for(i=0; i<=5; i++) printf("%10li ", (long int)pPSI_info_hdr->I4SCAL_A[i]);
  if(verbosemode == 1) printf("\n");

  p= buf+694;
  bdecode_obj( p, pPSI_info_hdr->NSC, 3*2 );
  if(verbosemode == 1) printf("CAMAC station of the singles scalers NSC = ");
  if(verbosemode == 1) for(i=0; i<=2; i++) printf("%3li ", (long int)pPSI_info_hdr->NSC[i]);
  if(verbosemode == 1) printf("\n");
    ...
    MUD_SEC_GRP* pMUD_histGrp;
{
    int status;
    int i;
    MUD_SEC_GEN_HIST_HDR* pMUD_histHdr = NULL;
    MUD_SEC_GEN_HIST_DAT* pMUD_histDat = NULL;
    int size;
    ...
    for( i = 0; i <= pPSI_info_hdr->NUMHIS-1; i++ )
    {
        ...
        // Read one histogram, with its padding
        lenpad = pPSI_info_hdr->KDAFHI * pPSI_info_hdr->LENDAF * 4;
        if( ( fread( buf, lenpad, 1, fin ) < 1 ) ) 
        {
            goto error;
        }
