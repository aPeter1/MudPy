 ...
 *    May 2021     DA  Add space for null termination (hdr struct is not treated as whole)
 *                     (Was not a bug before, but raised scary warnings.)
 */
...
typedef struct { 
	 ...
	 INT32	last_dac;	/* 0 */    /* last value of dac */
         ...
         char   experimenter[18+1];
	 INT16	lendatestrt;	/* 9 */   /* length of date-time in 2byte words */
	 char	dattimstart[18+1]; /* ' ' */ 
	 INT16	lendatend;	/* 0 */    /* this is zero unless run is ended normally */
	 char	dattimeend[18+1];	/* ' ' */ 
	 INT16	lentitle;	/* 40 */   
	 char	title[80+1];	/* ' ' */  /* should be at the end of the 91st 2byte word  */
	 INT16	lendatadesc;	/* 2 */ 
	 char	datadescfor[2+1];	/* ' F' */ 
	 ...
	 char	comment3[78+1];	/* ' ' */  /* this ends at the 255'th 2byte word */
